...
# Make the project root (parent of scripts/) importable
...
async def clear_existing_data() -> None:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(UserModel.image_file).where(UserModel.image_file.is_not(None)),
        )
...
async def update_post_dates() -> None:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(select(PostModel).order_by(PostModel.id))
